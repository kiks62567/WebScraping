...
number_URL = page_soup.findAll("span", {"class": "pagnDisabled"})
# Grabs URL for the next page in order to start process over again
URL_container = page_soup.findAll("a", {"class": "pagnNext"})
number_URL = number_URL[0].text
...
for x in range(int(number_URL)):
    ...
    for container in containers:
        try:
            ...
            number_rating = number_rating_containers[1].text
        ...
        try:
            ...
            f.write(brand + "," + name.replace(",", "|") + "," + price.replace(",", "") + "," + rating + "," +
                    number_rating.replace(",", "") + "," + stock + "," + og_price + "," + "\n")
        except(NameError, TypeError, IndexError, AttributeError):
            continue
